...
OUT_DIR="$REPO_ROOT/tests/data/reference/factor_workflow_effective_config"
...
run_case() {
  local name="$1"
  shift
  "$PYTHON_BIN" "$REPO_ROOT/src/eaggl.py" factor --hide-opts --deterministic --print-effective-config "$@" > "$OUT_DIR/${name}.json"
}
